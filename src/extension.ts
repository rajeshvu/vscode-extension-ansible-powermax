...
export async function activate(context: vscode.ExtensionContext) {
    ...
    all_snippets.forEach((item:Snippets) => {
        const disposable = vscode.commands.registerCommand(`ansible-powermax-snippets.search-${item.version}`, async () => {
            ...
            const selected = await vscode.window.showQuickPick<{ label: string; detail: string }>(commands, {
                matchOnDetail: true,
                placeHolder: `Search for Ansible PowerMax Snippet (${item.version})`,
                ignoreFocusOut: true,
            });
            if (!selected) {
                return;
